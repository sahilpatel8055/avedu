...
const StickyNavigation = ({ sections, className }: StickyNavigationProps) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Get the second section to determine when to show nav
      const secondSection = document.getElementById(sections[1]?.id);
      if (secondSection) {
        const rect = secondSection.getBoundingClientRect();
        // Show nav when second section is scrolled past
                ...
                <button
                  ...
                  className={cn(
                    ...
                    activeSection === section.id
                      ? "text-white bg-gradient-to-r from-orange-500 to-orange-600 shadow-lg scale-110"
